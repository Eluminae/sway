...
static void handle_touch_motion(struct sway_seat *seat,
	...
	double moved_x = seat->touch_x - point_event->ref_lx;
	double moved_y = seat->touch_y - point_event->ref_ly;
...
static void handle_touch_down(struct sway_seat *seat,
	...
	if (!surface) { // Must start from the surface
		return;
	}

	struct seatop_touch_point_event *point_event =
		calloc(1, sizeof(struct seatop_touch_point_event));
	sway_assert(point_event, "Unable to allocate point_event");
	point_event->touch_id = seat->touch_id;
	point_event->ref_lx = seat->touch_x;
	point_event->ref_ly = seat->touch_y;
...
void seatop_begin_touch_down(struct sway_seat *seat,
		struct wlr_surface *surface, struct wlr_event_touch_down *event,
		double sx, double sy) {
	seatop_begin_down_on_surface(seat, surface, sx, sy);
	handle_touch_down(seat, event);
}
...
void seatop_begin_down_on_surface(struct sway_seat *seat,
	...
	sway_assert(e, "Unable to allocate seatop_down_event");
	e->con = NULL;
	e->seat = seat;
	e->surface = surface;
